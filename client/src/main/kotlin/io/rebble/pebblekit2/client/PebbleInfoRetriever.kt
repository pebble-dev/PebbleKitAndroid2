...
public class PebbleInfoRetriever(private val context: Context) {
    ...
    public fun getConnectedWatches(): Flow<List<ConnectedWatch>> {
        return suspend {
            ...
            if (pebbleAppPackage == null) {
                ...
                val uri = PebbleKitProviderContract.ConnectedWatch.getContentUri(pebbleAppPackage)
                context.contentResolver.flowOfChanges(uri).map {
                    val cursor = context.contentResolver.query(
                        /* uri = */ uri,
                        /* projection = */ PebbleKitProviderContract.ConnectedWatch.ALL_COLUMNS.toTypedArray(),
                        /* selection = */ null,
                        /* selectionArgs = */ null,
                        /* sortOrder = */ null,
    ...
    private fun <T> Cursor.mapToList(mapper: () -> T): List<T> {
        return buildList {
            while (moveToNext()) {
                add(mapper())
            }
